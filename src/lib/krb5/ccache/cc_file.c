...
/* Read the cache file.  Set time offsets in context from the header if
...
fcc_start_seq_get(krb5_context context, krb5_ccache id, krb5_cc_cursor *cursor)
{
    ...
    ret = k5_cc_mutex_init(&fcursor->lock);
    if (ret)
        goto cleanup;
              ...
              krb5_creds *creds)
{
    ...
    k5_cc_mutex_lock(context, &data->lock);
    k5_cc_mutex_lock(context, &fcursor->lock);
    k5_buf_init_dynamic(&buf);
...
fcc_end_seq_get(krb5_context context, krb5_ccache id, krb5_cc_cursor *cursor)
{
    ...
    k5_cc_mutex_destroy(&fcursor->lock);
