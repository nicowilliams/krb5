...
def check_tabs(line, ln, allow_tabs):
    if not allow_tabs:
        if '\t' in line:
...
def check_cast(line, ln):
    ...
    # expressions without a real C parser.  As a heuristic, assume
    # (word) or (word *) are cast operators and anything else isn't.
    for m in re.finditer(r'\([\w]+(\s*\*)?\)(\s+)[^\s{]', line):
        if m.group(1) == '*':
            warn(ln, 'No space before * in cast operator')
        elif m.group(2):
            warn(ln, 'Space after cast operator')
...
if len(sys.argv) == 1:
    ...
    sys.stderr.write('Usage: krb5-check-cstyle.py [filename]\n')
    sys.exit(1)
